...
def minimax_decision(state, game):
    ...
    def max_value(state):
        ...
        minimax_counter = minimax_counter + 1
    ...
    return argmax(game.actions(state),key=lambda a: min_value(game.result(state, a))),minimax_counter
...
def alphabeta_search(state, game):
    ...
    def max_value(state, alpha, beta):
        ...
        alphabeta_counter = alphabeta_counter + 1
        if game.terminal_test(state):
            return game.utility(state, player)
        v = -infinity
    ...
    for a in game.actions(state):
        ...
        if v > best_score:
            best_score = v
            best_action = a
    return best_action,alphabeta_counter
...
class Game:
    ...
    def cur_per(self,state,*players):
        state = state
        while True:
            for player in players:
                move,alphabeta_counter = player(self, state)
